...
module.exports = function signOwnerAuth(req, res, next) {
  // This is an auth that applies specifically to signs
  // Ensures the user requesting a change to a sign is its owner



  console.log("CHECKING SIGN OWNER...");

  console.log("BODY IN OWNER AUTH IS: ", req.body);
  ...
  if(userId && signOwnerId && (userIdStr !== signOwnerIdStr) ) {  // verify not a sign sabbotage
    ...
    return res.status(401).json({error: true, msg: 'You do not appear to be the owner of this sign.'});
  }
