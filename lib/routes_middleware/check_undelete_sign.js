  ...
  if(signType) {
    ...
    Sign.find({userId: req.user._id, signType: req.query.signType}, function(err, signs) {
      ...
      if(signs && signs.length > 0) {
        var undeleteSigns = signs.filter(function(sign) { return shouldUndelete(sign); });
        function shouldUndelete(sign) {
          return (sign.profileId !== 'D') &&   // auth deletion requires new sign
                 (sign.status    === 'D');     // active signs don't count
        }
      ...
      function undeleteSignsAndSendResponse(signs) {
        ...
        signs.forEach(function(sign) {
          ...
          // last check before un-delete... probably not necessary (query uses signType & status checked)
          // REMOVE LATER IF WANT.
