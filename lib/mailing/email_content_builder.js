...
function buildPasswordResetHtmlEmailString(mailData) {
  var fullRedirect;
  var redirectBase    = mailData.host + '/requestpasswordchange/change?';
  ...
  if(process.env.NODE_ENV != 'prod') {
    ...
    fullRedirect = process.env.PASSWORD_RESET_BASE_HREF + 'requestpasswordchange/change?' + fullQueryParams;
  }
...
function buildConfirmationHtmlEmailString(mailData) {
  ...
  var redirectBase           = mailData.host + '/user/confirmation?';
  var emailQuery             = 'email='             + mailData.email;
