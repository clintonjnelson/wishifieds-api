...
module.exports = function(signBuilder) {
  ...
  function buildWordpressSign(profileWithSites) {
    ...
    profileWithSites.sites.forEach(function(elem, ind, orig) {
      ...
      newSignData.picUrl      = getPicUrl(elem) || profileWithSites.avatar_URL;
    ...
    var multiSignProfile        = cloneDeep(profileWithSites);  // use this as a base object
  ...
  function getPicUrl(wp) {
    try { return wp.logo.url; }
    catch (e) {
      console.log('Data access error for wordpress sign picUrl: ', e);
      return '';
    }
  }
