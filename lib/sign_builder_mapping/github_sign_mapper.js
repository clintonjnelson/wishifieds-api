...
module.exports = function(signBuilder) {
  ...
  function buildGithubSign(ghData) {
    ...
    var signProps = {
      ...
      knownAs:        (ghData.displayName || ghData.knownAs),  // TODO: VERIFY || is NOT BACKWARDS!
      linkUrl:        (ghData.profileUrl  || ghData.linkUrl),  // TODO: VERIFY || is NOT BACKWARDS!
      // ----------------- CUSTOM ---------------------
      followersCount: getFollowersCount(ghData),
      profileId:      ghData.id,
      picUrl:         getPicUrl(ghData),
    };
  ...
  function getFollowersCount(gh) {
    try { return gh._json.followers; }
    catch (e) {
      console.log('Data access error for github followers count: ', e);
      return '';
    }
  }
  function getPicUrl(gh) {
    try { return gh._json.avatar_url; }
    catch (e) {
      console.log('Data access error for github picUrl: ', e);
      return '';
    }
  }
