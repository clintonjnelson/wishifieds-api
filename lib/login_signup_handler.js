...
var isUsernameAllowed = require('./signpost_utils.js').isUsernameAllowed;
...
module.exports = function loginSignupHandler(req, userSignInfo, done) {
  ...
  function updateExistingUserOrMakeNewUserAndSign(user, apiData, done) {
    if(!user) {  // No user exists, create a new user & sign
      ...
      userInfo['email'              ] = apiData[apiFields.apiEmail] || (apiData.profile && apiData.profile[apiFields.apiEmail]);
      userInfo['username'           ] = getUsername(apiData);
  ...
  // --------- Helpers ---------
  function getUsername(apiData) {
    try {
      var username = apiData[apiFields.apiUsername] || (apiData.profile && apiData.profile[apiFields.apiUsername]);
      return isUsernameAllowed(username) ? username : null;
    }
    catch(e) {
      console.log('Could not get username in creating new user via oauth. Error is: ', e);
      return null;
    }
  }
};
