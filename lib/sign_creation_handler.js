...
function findOrMakeSign(req, userSignInfo, done) {
  ...
  function findOrMakeMultiSign() {
    ...
    function apiDataArrayCallback(err, fullApiData) {
      ...
      var fullDataWithMultiSigns = signBuilder[signType](fullApiData);

      var multiSigns = fullDataWithMultiSigns.multiSigns;
      if(multiSigns && multiSigns.length > 0) {
        ...
        multiSigns.forEach(function(unsavedSign, ind, orig) {
          ...
          SignModel.findOne(query, function(err, existingSign) {
            ...
            var userIdStr      = (user && user._id ) ? String(user._id) : null ;
            var signOwnerIdStr = (existingSign && existingSign.userId) ? String(existingSign.userId) : null;
            console.log("EXISTING SIGN MATCHING THE EXISTING MATCH QUERY IS: ", existingSign);

            // No existing sign or someone else's existing sign? create one
            ...
            // If sign found, but not for this user? create one
            else if ( userIdStr && signOwnerIdStr && (signOwnerIdStr !== userIdStr) ) {
              console.log("SIGN EXISTS, BUT ON ANOTHER USER, SO CREATE ON THIS USER ALSO");
  ...
  function findOrMakeStdSign() {
    ...
    SignModel.findOne( signQuery, function(err, sign) {
      ...
      console.log("SIGN OWNER IS: ", signOwnerIdStr);
  ...
  function changeSignOwner(sign, newOwner, done) {
    ...
    console.log("CHANGING SIGN OWNER NOW...");
