...
function findOrMakeSign(req, userSignInfo, done) {
  ...
  function changeSignOwner(sign, newOwner, done) {
    ...
    clearOauthVals['auth'][mongo.authType] = {}
    clearOauthVals['auth'][mongo.authType][mongo.authTypeId]  = null;
    clearOauthVals['auth'][mongo.authType][mongo.accessToken] = null;  // TODO: verify saving this way OK for ID Array

    console.log("ABOUT TO SAVE AUTH ON OTHER USER AS: ", clearOauthVals);
    // Update old owner. Update sign.
    // User.findByIdAndUpdate(sign.userId, {$set: clearOauthVals}, function(err, oldUser) {
