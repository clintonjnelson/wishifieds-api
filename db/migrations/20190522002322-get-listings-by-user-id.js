...
// Example Call:
// SELECT * FROM get_listings_by_user_id(6);
...
module.exports = {
  up: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query(`
      CREATE OR REPLACE FUNCTION get_listings_by_user_id(p_user_id INTEGER)
      RETURNS table(
        listingId INTEGER,
        userId INTEGER,
        username VARCHAR,
  ...
  down: (queryInterface, Sequelize) => {
    return queryInterface.sequelize.query(`
      DROP FUNCTION get_listings_by_user_id(integer);
    `);
  }
};
